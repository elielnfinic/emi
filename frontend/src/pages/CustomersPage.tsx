...
export function CustomersPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['customers', bid] }); resetForm(); setShowModal(false) },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, ...p }: Record<string, unknown>) => api.put(`/customers/${id}`, p),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['customers', bid] }); setEditCustomer(null) },
  })
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault()
    const payload = { businessId: bid, name, email: email || undefined, phone: phone || undefined, address: address || undefined, notes: notes || undefined }
    editCustomer ? updateMutation.mutate({ id: editCustomer.id, ...payload }) : createMutation.mutate(payload)
  }
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 divide-y divide-zinc-100 dark:divide-zinc-800 overflow-hidden">
          {customers.map(c => (
            <div key={c.id} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors group">
              {/* Avatar */}
              <div className={`w-9 h-9 rounded-xl bg-gradient-to-br ${avatarColor(c.id)} flex items-center justify-center text-white text-xs font-bold shrink-0`}>
                {getInitials(c.name)}
              </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                  {c.email && <span className="text-xs text-zinc-400 truncate">{c.email}</span>}
                  {c.phone && (
                  ...
                  {c.address && (
                    <span className="text-xs text-zinc-400 truncate max-w-[160px]">{c.address}</span>
                  )}
                </div>
              </div>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex gap-2 pt-2">
            <Button type="button" variant="secondary" onClick={() => { setShowModal(false); setEditCustomer(null); resetForm() }} className="flex-1">Annuler</Button>
            <Button type="submit" className="flex-1" loading={isPending}>{editCustomer ? 'Enregistrer' : 'Créer'}</Button>
          </div>
        </form>
      </Modal>
    </div>
